...
class Server extends \Home {
	...
	function Buy($f3) {
		...
		if (($saldo = $this->me->saldo)<$server->price) {
			$this->flash('Baki Anda Kurang, Hubungi Admin utk Deposit');
			$f3->reroute($f3->get('URI'));
		}
		if ( ! $account->check($f3->get('POST.user'))) {
			$this->flash('User Sudah Terdaftar, Sila Cuba Lagi');
			$f3->reroute($f3->get('URI'));
		}
		...
		if ($f3->exists('POST.pass',$pass)) {
			if ( ! \Check::Confirm('POST.pass')) {
				$this->flash('Password Tidak Sama');
				$f3->reroute($f3->get('URI'));
			}
		...
		if( ! $account->save()) {
			$this->flash('Harap Maaf, Sila Cuba Sebentar Lagi');
			$f3->reroute($f3->get('URI'));
		}
